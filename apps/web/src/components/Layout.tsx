...
=======
      className={`hidden md:fixed md:inset-y-0 md:left-0 md:z-30 md:flex group bg-[rgba(255,255,255,0.1)] backdrop-blur-xl border border-white/20 shadow-xl overflow-hidden transition-[width] duration-300 ${pinned ? 'md:w-52' : 'md:w-20 md:hover:w-52'}`}
>>>>>>> events-tab
    >
...
=======
              className={`flex items-center gap-3 px-3 py-2 rounded-lg text-white/60 hover:bg-[rgba(255,255,255,0.15)] hover:text-white ${pinned ? '' : 'justify-center md:group-hover:justify-start'}`}
>>>>>>> events-tab
            >
...
=======
      ...
      />
      <div
        className={`absolute inset-y-0 left-0 w-72 bg-[rgba(255,255,255,0.1)] backdrop-blur-xl border-r border-white/20 shadow-2xl transform transition-transform duration-300 ${drawerOpen ? 'translate-x-0' : '-translate-x-full'}`}
>>>>>>> events-tab
        role="dialog"
        aria-modal="true"
      >
...
=======
  return (
    <div className="relative min-h-screen w-screen overflow-hidden bg-[#0b0b11] text-white">
      ...
      <div className="pointer-events-none absolute inset-0">
        {/* Main bright powder blue */}
        <div className="absolute inset-0 bg-[radial-gradient(1200px_800px_at_20%_10%,rgba(135,180,255,0.6),transparent_70%)]" />
        {/* Secondary accent — light sky blue */}
        <div className="absolute inset-0 bg-[radial-gradient(900px_700px_at_80%_30%,rgba(100,200,255,0.45),transparent_65%)]" />
        {/* Very soft vignette for focus */}
        <div className="absolute inset-0 bg-[radial-gradient(65%_75%_at_50%_50%,transparent,rgba(0,0,0,0.12))]" />
      </div>

      {/* App chrome above the glow */}
      <div className="relative z-10">
        {Sidebar}
        {MobileDrawer}

        <header
          className={`fixed top-0 left-0 z-20 w-full h-20 border-b border-white/20 bg-[rgba(255,255,255,0.05)] backdrop-blur-lg flex items-center justify-between md:pl-[${RAIL_W}px] ${pinned ? `md:pl-[${FULL_W}px]` : ''}`}
        >
          ...
          <div className="flex-1 md:flex md:items-center md:justify-end">
            <div className="flex items-center gap-3 pr-4 md:pr-10">
              <button
                onClick={() => setPinned((v) => !v)}
                className="hidden md:flex p-2 rounded-lg bg-white/10 hover:bg-white/20 transition-colors"
                aria-label={pinned ? 'Collapse sidebar' : 'Expand sidebar'}
              >
                {pinned ? (
        ...
        <main
          className={`relative pt-20 min-h-screen md:ml-20 ${pinned ? 'md:ml-[200px]' : ''}`}
        >
>>>>>>> events-tab
